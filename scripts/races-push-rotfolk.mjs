#!/usr/bin/env node
/**
 * Upserts the Notion "Races" row for rotfolk (#694) from races-rotfolk-content.mjs,
...
import {
  ...
  pageBodyBlocks,
  pageProperties,
} from './races-rotfolk-content.mjs';
...
async function findRotfolkPageId() {
...
async function main() {
  let pageId = await findRotfolkPageId();
  ...
  if (!pageId) {
    ...
    pageId = created.id;
    console.log('Created Rotfolk page', pageId, created.url);
    return;
  }
  ...
  console.log('Updated Rotfolk page', pageId, page.url);
}
