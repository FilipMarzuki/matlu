#!/usr/bin/env node
/**
 * Upserts the Notion "Races" row for Lövfolk (#735) from races-lovfolk-content.mjs,
 * then run `npm run races:sync`. Requires NOTION_API_KEY.
 *
 * Sets Allied with: Pandor (Compact of Knowing).
 * Sets In tension with: Bergfolk (hardwood dispute).
...
import {
  ...
  pageBodyBlocks,
  pageProperties,
} from './races-lovfolk-content.mjs';
...
async function main() {
  const properties = {
    ...
    'In tension with': {
      relation: [{ id: RACE_PAGE_IDS.bergfolk }],
    },
  };

  const patch = await fetch(`https://api.notion.com/v1/pages/${LOVFOLK_PAGE_ID}`, {
    method: 'PATCH',
    headers: postHeaders,
  ...
  await archiveChildBlocks(LOVFOLK_PAGE_ID);
  await appendBlocks(LOVFOLK_PAGE_ID, pageBodyBlocks);

  const info = await fetch(`https://api.notion.com/v1/pages/${LOVFOLK_PAGE_ID}`, {
    headers: baseHeaders,
  });
  const page = await info.json();
  console.log('Updated Lövfolk page', LOVFOLK_PAGE_ID, page.url);
}
