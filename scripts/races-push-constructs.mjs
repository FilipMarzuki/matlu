 ...
 *
 * Resolves `Default Culture` → Workshop-Towns (workshop-collective) by querying the Cultures DB
 * (Name match, else `id` rich_text), or `NOTION_WORKSHOP_COLLECTIVE_CULTURE_PAGE_ID` if set.
 */
import {
...
async function findWorkshopCollectivePageId() {
  const fromEnv = process.env.NOTION_WORKSHOP_COLLECTIVE_CULTURE_PAGE_ID?.trim();
  if (fromEnv) {
    console.log('Using NOTION_WORKSHOP_COLLECTIVE_CULTURE_PAGE_ID');
    return fromEnv;
  }
  let id = await queryOneCulturePage({
    property: 'Name',
    title: { equals: 'Workshop-Towns' },
  });
  if (id) return id;
  id = await queryOneCulturePage({
    property: 'id',
    rich_text: { equals: 'workshop-collective' },
  });
  return id;
}
...
async function main() {
  const defaultCultureId = await findWorkshopCollectivePageId();
  if (!defaultCultureId) {
    console.warn(
      'Could not resolve Default Culture (Workshop-Towns / workshop-collective). ' +
        'Set NOTION_WORKSHOP_COLLECTIVE_CULTURE_PAGE_ID, or add Name/id on the Culture row. ' +
  ...
  properties['In tension with'] = { relation: [{ id: BERGFOLK_RACE_PAGE_ID }] };
  if (defaultCultureId) {
