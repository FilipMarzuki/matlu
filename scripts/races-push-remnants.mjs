#!/usr/bin/env node
/**
 * Upserts the Notion "Races" row for Remnants (#748) from races-remnants-content.mjs,
...
import {
  ...
  pageBodyBlocks,
  pageProperties,
} from './races-remnants-content.mjs';
...
async function main() {
  // The Remnants page already exists as a stub; patch it directly.
  const patch = await fetch(`https://api.notion.com/v1/pages/${REMNANTS_PAGE_ID}`, {
    method: 'PATCH',
    headers: postHeaders,
  ...
  await archiveChildBlocks(REMNANTS_PAGE_ID);
  await appendBlocks(REMNANTS_PAGE_ID, pageBodyBlocks);

  const info = await fetch(`https://api.notion.com/v1/pages/${REMNANTS_PAGE_ID}`, {
    headers: baseHeaders,
  });
  const page = await info.json();
  console.log('Updated Remnants page', REMNANTS_PAGE_ID, page.url);
}
