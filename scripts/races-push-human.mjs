#!/usr/bin/env node
/**
 * Upserts the Notion "Races" row for human (#689) from races-human-content.mjs,
...
async function findHumanPageId() {
...
async function main() {
  let pageId = await findHumanPageId();
  ...
  if (!pageId) {
    ...
    pageId = created.id;
    console.log('Created Human page', pageId, created.url);
    return;
  }
  ...
  console.log('Updated Human page', pageId, page.url);
}
