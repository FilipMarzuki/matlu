...
export class GameScene extends Phaser.Scene {
  ...
  private createHudAndOverlay(): void {
    ...
    // Full-screen corruption tint — deep violet/black keeps corrupted worlds
    // threatening without flattening biome hue into neutral grey.
    this.overlay = this.add
      .rectangle(sw / 2, sh / 2, sw, sh, 0x16051f, 0.44)
      .setScrollFactor(0)
      .setDepth(50);
  ...
  private applyWorldTint(percent: number): void {
    const ratio = Phaser.Math.Clamp(percent / 100, 0, 1);
    this.overlay.setAlpha(0.44 * (1 - ratio));
  }
