 ...
 *
 * Session token = HMAC-SHA256(CODEX_ADMIN_PASSWORD, 'matlu-admin-v1') in hex.
 * This means:
 *  - The cookie doesn't contain the plaintext password
 *  - Changing CODEX_ADMIN_PASSWORD instantly invalidates all existing sessions
...
/** Validate the submitted password against CODEX_ADMIN_PASSWORD env var. */
export function validatePassword(submitted: string): boolean {
  const expected = import.meta.env.CODEX_CODEX_ADMIN_PASSWORD ?? '';
...
export function isAuthorized(cookieHeader: string | null): boolean {
  const password = import.meta.env.CODEX_CODEX_ADMIN_PASSWORD ?? '';
