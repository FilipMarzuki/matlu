...
## STEP 8 — COMMUNITY ENTITIES: ENRICH WITH SUBMISSION DATA

After filling in all standard entities, also process entities in the registry
that have `"source": "community"` — these were created from creature submissions
and their `designNotes` should draw on the full submission record rather than
just the sparse registry entry.

### 8a. Identify community entities needing notes

Scan `src/entities/entity-registry.json` for entries where:
- `source === "community"`, AND
- `submissionId` is set (non-null), AND
- `designNotes` is null or incomplete

### 8b. Fetch the submission record from Supabase

For each community entity, fetch its source submission using `submissionId`:

```bash
curl -s -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" \
  "$SUPABASE_URL/rest/v1/creature_submissions?id=eq.{entity.submissionId}&select=creature_name,creator_name,maker_age,world_name,kind_size,kind_movement,kind_diet,kind_solitary,habitat_biome,habitat_climate,behaviour_threat,special_ability,lore_description,lore_origin,graphics_notes,balance_tier,biome_affinity"
```

(`SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are available as env vars.)

### 8c. Write design notes using submission data as primary source

Use the same `designNotes` schema and writing rules as STEP 5, but:

- **`sprite`**: use `graphics_notes` from the submission as the primary brief if
  non-empty (it was written by the admin specifically for PixelLab). Then expand
  to 2–4 sentences with `kind_size`, movement, and world-consistent colour palette.
- **`animations`**: infer from `behaviour_threat` and `special_ability`.
  "Harmless" → no attack anim needed (idle + walk + death minimum).
  "Aggressive" → full idle + walk + attack + hurt + death set.
- **`sounds`**: ground in `world_name` (earth = mechanical/glitch, spinolandet = organic/bio).
  Reference `behaviour_threat` for aggro sound character.
- The child's `lore_description` is gold — read it and let the creature's own story
  inform the design notes. Honour the creator's vision.

### 8d. Update the registry

Write the filled `designNotes` back to `entity-registry.json`, same as STEP 6.

## STEP 9 — REPORT

Print a summary:
- How many entities had designNotes before this run
- How many were filled in this run (standard + community)
...
- Any community entities processed and which submission records were used
